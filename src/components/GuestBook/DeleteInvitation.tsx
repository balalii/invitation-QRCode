...
import {  deleteInvitation } from '@/actions/invitation.action';
import toast from 'react-hot-toast';

export default function DeleteInvitation({id}:{id:string}) {
     const [dialogOpen, setDialogOpen] = React.useState(false);
   const [errorMessage, formAction] = useFormState(deleteInvitation, undefined);
    ...
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
      ...
      <DialogContent className="py-12 lg:py-8 max-h-screen overflow-y-auto">
        <form action={formAction}>
          <DialogHeader>
            <DialogTitle className="text-center lg:text-left ">Hapus Undangan</DialogTitle>
          ...
          <div className="flex flex-row items-center !space-y-0 justify-end space-x-4 pt-8">
            <ButtonForm type="submit" className={`m-0 w-full lg:w-fit lg:!px-5 bg-[var(--primary-color)]`} label={'Hapus Undangan'} />
          </div>
        </form>
      </DialogContent>
    </Dialog>
