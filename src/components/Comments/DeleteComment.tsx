...
import { deleteInvitationGreeting } from '@/actions/invitationGreeting.action';

export default function DeleteComment({id}:{id:string}) {
     const [dialogOpen, setDialogOpen] = React.useState(false);
   const [errorMessage, formAction] = useFormState(deleteInvitationGreeting, undefined);
    ...
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
      ...
      <DialogContent className="py-12 lg:py-8 max-h-screen overflow-y-auto">
        <form action={formAction}>
          <DialogHeader>
            <DialogTitle className="text-center lg:text-left ">Hapus Komentar</DialogTitle>
          ...
          <div className="flex flex-row items-center !space-y-0 justify-end space-x-4 pt-8">
            <ButtonForm type="submit" className={`m-0 w-full lg:w-fit lg:!px-5 bg-[var(--primary-color)]`} label={'Hapus Komentar'} />
          </div>
        </form>
      </DialogContent>
    </Dialog>
